...
import { addingBannerToChunk, setCompressionOptions, contextAnswer, cssPreprocessors, entryPointsAnswer, fontsExtensions, chooseStaticFilesLoader, htmlPreprocessorsAnswer, imageExtensions, integrationInstruments, isAvoidErrorStyles, isCacheWebpack, isClosureLibrary, isCompressionAnswer, isCopyStaticFiles, isCreateChromeProfileFile, isCssPreprocessorsAnswer, isCsvExtension, isDevServerAnswer, isDiscoverPreviousCompilation, isEnvironmentVariables, isFontsAnswer, isGlobalVariableAnswer, isHMRAnswer, isHtmlPreprocessorAnswer, isIgnoreSomeFiles, isImageExtensionAnswer, isIntegrationInstrument, isLazyLoading, isLocalizeAnswer, isPwaSupport, isSplitBundlesThroughDLL, isSplittingChunks, isXmlExtension, isYamlExtension, outputDir, setAliasAnswer, setEnvironmentVariables, setFilesCatalogesCopy, setGlobalVariable, setLocalizeDetails, setMaximumChunkSize, setMinimumChunkSize, supportFromCoffeScriptAnswer, supportSplitBundlesThroughDLL, staticLoader, isCopyPlugin, isCleanPlugin, fontsOutDir, } from "./answers";
...
async function WebpackConfigCustom(presetType, mode) {
    ...
    const entryPointWrite = await entryPointsAnswer(presetType, contextPrintWrite.question_context);
    const setAliasPathes = await setAliasAnswer(contextPrintWrite.question_context);
    ...
    const htmlPreprocessors = await htmlPreprocessorsAnswer(isHtmlPreprocessorSupport.question_is_html_preprocessor);
    const isCssPreprocessor = await isCssPreprocessorsAnswer();
    const cssPreprocessorsSupport = await cssPreprocessors(isCssPreprocessor.question_is_css_preprocessor);
    const isStaticLoaderSupport = await staticLoader();
    const chooseStaticFilesLoaderSupport = await chooseStaticFilesLoader(isStaticLoaderSupport.question_static_loader);
    const isImageExtension = await isImageExtensionAnswer();
    const imageExtensionsSupport = await imageExtensions(isImageExtension.question_is_image_extensions);
    ...
    const isBannerSupport = await addingBannerToChunk(isSplittingChunksSupport.question_is_splitting_chunks);
    const isClosureLibrarySupport = await isClosureLibrary();
    const isGlobalVariableSupport = await isGlobalVariableAnswer();
    const SetGlobalVariableSupport = await setGlobalVariable(isGlobalVariableSupport.question_is_global_variable_answer);
    const isSplitBundlesThroughDLLSupport = await isSplitBundlesThroughDLL();
    const supportSplitBundlesDLL = await supportSplitBundlesThroughDLL(isSplitBundlesThroughDLLSupport.question_is_split_bundles_dll);
    const isEnvironmentVariablesSupport = await isEnvironmentVariables();
    const setEnvironmentVariableNameAndValueSupport = await setEnvironmentVariables(isEnvironmentVariablesSupport.question_environment_variables);
    const isDiscoverPreviousCompilationSupport = await isDiscoverPreviousCompilation();
    const isLocalizeSupport = await isLocalizeAnswer();
    const setLocalizeDetailsSupport = await setLocalizeDetails(isLocalizeSupport.question_is_localize);
    const setMinimumChunkSizeSupport = await setMinimumChunkSize(isSplittingChunksSupport.question_is_splitting_chunks);
    const setMaximumChunkSizeSupport = await setMaximumChunkSize(isSplittingChunksSupport.question_is_splitting_chunks);
    ...
    const setIntegrationSupport = await integrationInstruments(isIntegrationSupport.question_is_integration);
    const isHMRSupport = await isHMRAnswer();
    const isCompressionSupport = await isCompressionAnswer();
    const setCompressionOptionsSupport = await setCompressionOptions(isCompressionSupport.question_is_compression_answer);
    ...
    const setFilesCatalogesCopySupport = await setFilesCatalogesCopy(isCopyStaticFilesSupport.is_copy_static_files);
...
async function WebpackConfigOptions(presetType, mode) {
    ...
    const entryPointWrite = await entryPointsAnswer(presetType, contextPointWrite.question_context);
    const aliasPathWrite = await setAliasAnswer(contextPointWrite.question_context);
    ...
    const checkLangPreset = await checkPresetFrameworkConfig(presetType);
    const checkTsConfigPreset = await checkPresetTsConfig(checkLangPreset === null || checkLangPreset === void 0 ? void 0 : checkLangPreset.langForFramework.question_12);
